...
mdevctl start -u $EEE -p 0000:$PCI --type $DEV
mdevctl start -u $FFF -p 0000:$PCI --type $DEV
mdevctl start -u $GGG -p 0000:$PCI --type $DEV
mdevctl start -u $HHH -p 0000:$PCI --type $DEV
OUTPUT=$(mdevctl list)
echo "${OUTPUT}"
